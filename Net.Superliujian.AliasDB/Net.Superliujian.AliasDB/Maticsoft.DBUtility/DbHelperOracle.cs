...
namespace Maticsoft.DBUtility
{
    ...
    public abstract class DbHelperOracle
    {
        ...
        {
            //TODO:Not Test yet
            DataSet ds = Query(string.Format("select COLUMN_NAME from user_tab_columns where table_name ={0}", table));
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                fields.Add(dr["COLUMN_NAME"].ToString());

            }
            ds = Query(string.Format("select col.column_name from user_constraints con,  user_cons_columns col where con.constraint_name = col.constraint_name and con.constraint_type='P' and col.table_name = {0}", table));
