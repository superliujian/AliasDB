    ...
    public abstract class DbHelperOleDb
    {
        ...
        public static void GetColumns(string table, ref List<string> fields, ref string primaryKey)
        {
            ...
            try
            {
                ...
                DataTable schemaTable = conn.GetOleDbSchemaTable(OleDbSchemaGuid.Columns, new object[] { null, null, table, null });
                foreach (DataRow dr in schemaTable.Rows)
                {
                    fields.Add(dr["COLUMN_NAME"].ToString());
                }

                schemaTable = conn.GetOleDbSchemaTable(OleDbSchemaGuid.Primary_Keys, new object[] { null, null, table });
                if (schemaTable.Rows.Count > 0)
                    primaryKey = schemaTable.Rows[0]["COLUMN_NAME"].ToString();
